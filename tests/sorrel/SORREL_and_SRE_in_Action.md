# SORREL and CRE in Action

## Overview

SORREL operates on a simple but powerful principle:
...
# The SORREL Development Loop

In a SORREL-driven workflow, prompts form an iterative loop between the AI agent and the execution environment.
...
This discovery process becomes the **foundation for SDD testing**.
...
By requiring test classes first, SORREL forces the agent to **validate assumptions before building systems**.
...
# The Role of the SORREL Restrictions Engine
...
A SORREL project begins with a directory structure designed for AI collaboration.
...
These directories represent the core artifacts of the SDD system.
...
SORREL significantly improves reliability by aligning with how AI systems actually work.
...
SORREL addresses these limitations by making the development process **incremental and feedback-driven**.

---

# SORREL Builds SORREL

An important philosophical element of the project is that **SORREL itself was developed using SORREL principles**.
...
The resulting artifacts now form the initial **SDD core tests**.
...
SDD replaces these concepts with:
...
SORREL assumes that **AI agents must learn the system first**.
...
SDD creates a development process where AI agents progressively acquire knowledge about their environment before constructing systems.
...
If you'd like, the next very powerful step would be designing the **SORREL Facts Specification**, because facts become the **memory layer for AI agents working across multiple prompts and sessions**. That is where SORREL starts turning into a full **AI-native development operating system**.