...
// Function to find the 'sdd' directory by looking upwards from the current executable's path
fs::path find_sdd_directory() {
    fs::path current_path = fs::current_path();
    while (current_path.has_parent_path()) {
        if (fs::exists(current_path / "sdd") && fs::is_directory(current_path / "sdd")) {
            return current_path / "sdd";
        }
...
int main(int argc, char* argv[]) {
    if (argc > 1 && std::string(argv[1]) == "sip") {
        ...
        std::cout << "SORREL Facts" << std::endl;
        std::cout << std::endl;
        // This command no longer uses hardcoded paths for this sip
        std::cout << "(Fact file discovery not yet implemented, use 'discover sdd')" << std::endl;
