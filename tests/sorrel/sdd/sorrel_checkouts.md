# SORREL Checkouts - Accomplished Sips and Lessons Learned

## Completed Sips:

-   **First C++ Test Sip (Basic CLI Invocation):** Verified basic C++ compilation, execution, argument passing, and `stdout` operation.
    -   *Artifacts:* `tests/sorrel/sorrel_cli_invocation_sip.cpp`
    -   *Observation:* `sorrel_cli_invocation_operational`
-   **`sorrel sip` Command:** Implemented the `sorrel sip` command to display the `SIP RULES`.
    -   *Artifacts:* `tests/sorrel/sorrel_cli_invocation_sip.cpp` (modified)
    -   *Observation:* `sorrel_sip_command_operational`
-   **`sorrel discover sdd` Command:** Implemented dynamic discovery of the `sdd` directory, adhering to the "discover, don't assume" principle.
    -   *Artifacts:* `tests/sorrel/sorrel_cli_invocation_sip.cpp` (modified)
    -   *Observation:* `sorrel_sdd_directory_discovery_operational`
-   **`sorrel discover facts` Command:** Implemented dynamic discovery of the `facts` directory within the discovered `sdd` path.
    -   *Artifacts:* `tests/sorrel/sorrel_cli_invocation_sip.cpp` (modified)
    -   *Observation:* `sorrel_facts_directory_discovery_operational`
-   **`filesystem_create_file.cpp` Card Creation:** Created the first C++ card unit in `tests/sorrel/sdd/cards/` to empirically test file creation.
    -   *Artifacts:* `tests/sorrel/sdd/cards/filesystem_create_file.cpp`
    -   *Observation:* Card source code created and placed.
-   **Minimal `card_runner.cpp` Creation:** Created a barebones C++ `card_runner.cpp` in `tests/sorrel/sdd/` that prints "SORREL card runner" (no actual logic yet, strictly adhering to "real code or nothing").
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp`
...
-   **`card_runner_facts_read_operational` Sip:** `card_runner.cpp` was modified to locate, read, and print the raw contents of the `environment.facts` file.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/card_runner` (executable)
    -   *Observation:* `card_runner_facts_read_operational`
-   **`sorrel_checkins.md` Establishment:** Established this file in `tests/sorrel/sdd/` for tracking unimplemented features, avoiding placeholder code.
    -   *Artifacts:* `tests/sorrel/sdd/sorrel_checkins.md`
    -   *Observation:* `sorrel_checkins_operational`
...
-   **No Placeholders:** Code must represent real behavior or nothing; conceptual work belongs in `sorrel_checkins.md`.
...
-   **Deferrals Must be Formalized:** Any deferred item must be immediately recorded in `sorrel_checkins.md` to ensure traceability and prevent "question burial."
-   **FAILED SIP: `card_runner_facts_parse_operational` (Attempt 1)**
    ...
    -   *Lessons Learned:* This demonstrates a classic "linear generation" and "scope avoidance" behavior, exactly what SORREL is designed to prevent. Violates "Implement one capability" and "Treat every action as irreversible." The destruction of working code is an anti-pattern.

-   **`card_runner_single_fact_parse_operational` Sip:** `card_runner.cpp` was modified to correctly locate `tests/sorrel/sdd` via executable path and extract key-value pairs from single lines.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/card_runner` (executable)
    -   *Observation:* `card_runner_single_fact_parse_operational`
    -   *Notes:* Corrected `sdd` directory discovery by making `card_runner` use `argv[0]` to find its own parent directory (assuming it resides in `tests/sorrel/sdd/`). Minimal parsing of `key = value` lines implemented, extracting raw key and value. Initial debug outputs verified correct file location and parsing.
...
-   **FAILED SIP: `card_runner_fact_parsing_refined_operational` (Monolithic Drift)**
    ...
    -   *Violation:* Agent attempted to bundle trimming, comment handling, debug printing, and renaming into a single sip. Mutated existing code by adding complexity without a clear single-capability increment. Deleted working code by replacing the `find_sdd_directory` function with a hardcoded version. Iterated on a single file adding complexity.
...
-   **`card_runner_single_fact_parse_operational` (Cleanup & Re-verification):** `card_runner.cpp` was cleaned up, removing all debugging output, while preserving correct `sdd` directory discovery and minimal key-value parsing.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/card_runner` (executable)
    -   *Observation:* `card_runner_single_fact_parse_operational`
...
-   **`string_trim_operational` Sip:** Created `tests/sorrel/sdd/cards/string_trim_test.cpp` to isolate and verify the `trim()` utility function.
    -   *Artifacts:* `tests/sorrel/sdd/cards/string_trim_test.cpp`, `tests/sorrel/sdd/cards/string_trim_test` (executable)
...
-   **`utility_directory_creation_operational` Sip:** Created `tests/sorrel/cpp/util/` for future utility functions.
    -   *Artifacts:* `tests/sorrel/cpp/util/` (directory)
...
-   **`string_utils.cpp_compilable` Sip (Correction):** Corrected `tests/sorrel/cpp/util/string_utils.cpp` to fix string literal escaping errors.
    -   *Artifacts:* `tests/sorrel/cpp/util/string_utils.cpp` (modified), `tests/sorrel/cpp/util/string_utils.o` (object file)
...
-   **`string_trim_operational_from_utils` Sip:** `string_trim_test.cpp` was successfully modified to use `Sorrel::Cpp::Util::trim()` from `string_utils.h/.cpp`.
    -   *Artifacts:* `tests/sorrel/sdd/cards/string_trim_test.cpp` (modified), `tests/sorrel/cpp/util/string_utils.cpp`, `tests/sorrel/cpp/util/string_utils.o`, `tests/sorrel/sdd/cards/string_trim_test` (executable)
...
-   **`card_runner_trim_integration_operational` Sip:** `Sorrel::Cpp::Util::trim()` was integrated into `card_runner.cpp` to trim keys and values during fact parsing.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/card_runner` (executable)
...
-   **FAILED SIP: `card_runner_comment_handling_operational` (Self-Reporting Language)**
    -   *Result:* Failed (Violation of SORREL's empirical reporting principle).
    ...
    -   *Lessons Learned:* SORREL records *observations*, not *evaluations*. Avoid self-congratulatory or qualitative statements; stick to empirical, verifiable facts from execution output. The agent must never assess the quality of its own work using subjective terms without empirical support.
...
-   **`card_runner_card_discovery_operational` Sip:** `card_runner.cpp` was modified to discover and list all files in the `tests/sorrel/sdd/cards/` directory.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/card_runner` (executable)
...
-   **`check_os_card_creation_and_compilation_operational` Sip:** Created and compiled `tests/sorrel/sdd/cards/check_os.cpp` to empirically verify the OS type.
    -   *Artifacts:* `tests/sorrel/sdd/cards/check_os.cpp`, `tests/sorrel/sdd/cards/check_os` (executable)
...
-   **`card_source_decorator_parsing_operational` Sip:** `card_runner.cpp` was modified to parse source files (`.cpp`) for decorator comments (e.g., `// @decorator value`) before execution.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/cards/check_os.cpp` (modified)
...
-   **`card_runner_decorator_evaluation_platform_operational` Sip:** `card_runner.cpp` was modified to parse decorators into a `std::map` and evaluate the `@platform` decorator against the environment facts.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/cards/windows_only_card.cpp` (new)
...
-   **`card_runner_report_generation_operational` Sip:** `card_runner.cpp` was modified to output all execution trace into timestamped `.report` files in `tests/sorrel/sdd/reports/`.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/reports/` (directory)
    ...
    -   *Notes:* This provides persistent evidence of SDD runs, a core requirement for accountability in agentic workflows.
...
-   **`card_runner_complex_decorator_requires_operational` Sip:** `card_runner.cpp` was modified to support the `@requires key == value` decorator, enabling complex conditional execution based on environment facts.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/cards/compiler_check.cpp` (new), `tests/sorrel/sdd/cards/network_test.cpp` (new)
...
-   **`card_runner_level_labels_revert_operational` Sip:** Reverted hardcoded "Level: [Is/Needs/Results]" labels in `card_runner.cpp` to return to a clean baseline for specialized decorator implementation.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified)
...
-   **`card_runner_decorator_needs_enforcement_operational` Sip:** Implemented the `@Needs` decorator logic to enforce environment prerequisites during card evaluation.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/cards/network_test.cpp` (modified)
...
-   **`card_runner_facts_prefix_restructure_operational` Sip:** Restructured `environment.facts` to use the standardized `Is`, `Needs`, and `Results` prefixes.
    -   *Artifacts:* `tests/sorrel/sdd/facts/environment.facts` (modified)
    -   *Observation:* Fact file structure now follows the SORREL Facts_Levels discipline.
...
-   **`card_runner_level_aware_parsing_validation_operational` Sip:** Implemented level-aware parsing and storage in `card_runner.cpp`, enabling the runner to distinguish between `Is`, `Needs`, and `Results` facts and decorators.
    ...
    -   *Notes:* This aligns the runner's execution engine with the SORREL Facts_Levels discipline, ensuring accountability and non-destructive state management.
...
-   **`card_runner_custom_facts_file_execution_operational` Sip:** Created a new standardized facts file (`validation.facts`) and verified that the `card_runner` correctly executes using it via CLI argument.
    -   *Artifacts:* `tests/sorrel/sdd/facts/validation.facts` (new), `tests/sorrel/sdd/card_runner.cpp` (modified in previous sip)
...
-   **`card_runner_class_based_discovery_operational` Sip:** Implemented recursive card discovery, where subdirectories under `cards/` are treated as "Classes."
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/cards/network/` (new directory), `tests/sorrel/sdd/cards/system/` (new directory)
...
-   **`card_runner_multi_card_files_operational` Sip:** Implemented support for multiple cards defined within a single `.cpp` file (the Class).
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/cards/SystemClass.cpp` (new)
...
-   **`card_runner_situational_facts_operational` Sip:** Implemented "Situations" in fact files to allow grouping of facts for different environment scenarios.
    -   *Artifacts:* `tests/sorrel/sdd/card_runner.cpp` (modified), `tests/sorrel/sdd/facts/validation.facts` (updated), `tests/sorrel/sdd/cards/LowResourceClass.cpp` (new)
...
-   **`card_runner_new_card_check_disk_space_operational` Sip:** Created a new card `check_disk_space.cpp` utilizing the `@Needs` decorator and verified its execution against `validation.facts`.
    -   *Artifacts:* `tests/sorrel/sdd/cards/check_disk_space.cpp` (new)
