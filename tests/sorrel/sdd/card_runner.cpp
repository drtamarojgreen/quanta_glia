...
// Function to find the 'sdd' directory based on the executable's path
fs::path get_executable_sdd_directory(const char* argv0) {
    fs::path executable_path = fs::canonical(argv0); // Get absolute path to executable
    fs::path executable_parent_path = executable_path.parent_path(); // Should be tests/sorrel/sdd/

    // Assuming card_runner is directly inside tests/sorrel/sdd/
...
int main(int argc, char* argv[]) {
    ...
    fs::path sdd_dir = get_executable_sdd_directory(argv[0]);
    if (sdd_dir.empty()) {
        std::cerr << "Error: 'sdd' directory not found from executable path." << std::endl;
        return 1;
    }

    // Prepare report directory and file
    fs::path reports_dir = sdd_dir / "reports";
    ...
    log("SORREL card runner");

    if (sdd_dir.empty()) {
        log("Error: 'sdd' directory not found from executable path.", true);
        return 1;
    }
    ...
    if (argc > 1) {
        ...
        if (facts_file_path.is_relative()) {
            facts_file_path = sdd_dir / "facts" / facts_file_path;
        }
    } else {
        facts_file_path = sdd_dir / "facts" / "environment.facts";
    }
    ...
    if (facts_file.is_open()) {
        ...
        while (std::getline(facts_file, line)) {
            std::string trimmed_line = Sorrel::Cpp::Util::trim(line);

            if (trimmed_line.empty()) continue;

            if (trimmed_line.find("Situation:") == 0) {
                current_situation = Sorrel::Cpp::Util::trim(trimmed_line.substr(10));
            ...
            if (level_end != std::string::npos) {
                ...
                std::string rest = Sorrel::Cpp::Util::trim(trimmed_line.substr(level_end + 1));
                size_t eq_pos = rest.find('=');
                if (eq_pos != std::string::npos) {
                    std::string key = Sorrel::Cpp::Util::trim(rest.substr(0, eq_pos));
                    std::string value = Sorrel::Cpp::Util::trim(rest.substr(eq_pos + 1));
                ...
                if (filename.find('.') == std::string::npos) {
                    ...
                    if (fs::exists(source_path)) {
                        ...
                        while (std::getline(source_file, source_line)) {
                            std::string trimmed_source = Sorrel::Cpp::Util::trim(source_line);
                            if (trimmed_source.find("// @Card:") == 0) {
                                std::string card_name = Sorrel::Cpp::Util::trim(trimmed_source.substr(9));
    ...
    for (const auto& card : discovered_cards) {
        ...
        if (card.decorators.count("Is")) {
            ...
            if (eq_pos != std::string::npos) {
                std::string key = Sorrel::Cpp::Util::trim(val.substr(0, eq_pos));
                std::string expected = Sorrel::Cpp::Util::trim(val.substr(eq_pos + 2));
        ...
        if (should_execute && card.decorators.count("requires")) {
            ...
            if (eq_pos != std::string::npos) {
                std::string key = Sorrel::Cpp::Util::trim(req.substr(0, eq_pos));
                std::string value = Sorrel::Cpp::Util::trim(req.substr(eq_pos + 2));
        ...
        if (should_execute) {
            ...
            while (std::getline(output_stream, out_line)) {
                std::string trimmed_out = Sorrel::Cpp::Util::trim(out_line);
                size_t eq_pos = trimmed_out.find('=');
                if (eq_pos != std::string::npos) {
                    std::string key = Sorrel::Cpp::Util::trim(trimmed_out.substr(0, eq_pos));
                    std::string value = Sorrel::Cpp::Util::trim(trimmed_out.substr(eq_pos + 1));
